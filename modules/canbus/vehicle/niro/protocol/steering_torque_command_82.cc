...
/*
void Steeringtorquecommand82::set_controller(VehicleController * controller)
{
...
void Steeringtorquecommand82::set_p_steering_torque_command(uint8_t* data,
  ...
  if(message_manager_) {// && controller_) {
    // Read current angle
    ...
    double Kp = 0.006, Ki = 0.00013, Kd = 0.0; // roscco_apollo: 0.016, 0.004, 0.001
    double error = current_angle - angle_target;
    ...
    double Iout = 0.0;
//    if(controller_->driving_mode() == Chassis::COMPLETE_AUTO_DRIVE) {
        integral_ += error;
        if(integral_ * Ki > 0.2) integral_ = 0.2/Ki;
        else if(integral_ * Ki < -0.2) integral_ = -0.2/Ki;
        Iout = integral_ * Ki;
//    }
    ...
    torque_command_average_ = calc_exponential_average(torque_command_average_, torque_command, 0.1);
    AINFO << "Pout=" << Pout << ",Iout=" << Iout << ",Dout=" << Dout << ",torque=" << torque_command_average_;

    pre_error_ = error;
  }

  float steering_torque_command_f = ProtocolData::BoundedValue(-1.0, 1.0, torque_command_average_);
